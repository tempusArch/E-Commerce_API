...
public class AddIntoCartItem {
    public record AddIntoCartItemCommand(CartItem CartItem) : IRequest<SingleCartItemDto>;

    public class Handler : IRequestHandler<AddIntoCartItemCommand, SingleCartItemDto> {
        ...
        public async Task<SingleCartItemDto> Handle(AddIntoCartItemCommand command, CancellationToken cancellationToken) {
            ...
            CartItem? theCartItem = await _context.CartItemTable.FirstOrDefaultAsync(
                x => x.ProductId == command.CartItem.ProductId && x.CartId == command.CartItem.CartId,
                cancellationToken);
