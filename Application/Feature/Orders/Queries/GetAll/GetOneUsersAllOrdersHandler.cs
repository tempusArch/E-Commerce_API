...
public class GetOneUsersAllOrdersHandler : IRequestHandler<GetOneUsersAllOrdersQuery, IEnumerable<OrderDto>> {
    ...
    public async Task<IEnumerable<OrderDto>> Handle(GetOneUsersAllOrdersQuery query, CancellationToken cancellationToken) {
        ...
        return theOrderRisuto.Select(x => new OrderDto {
            OrderId = x.Id,
            AllCartItems = new AllCartItemsDto(
                x.OrderItemRisuto.Select(
                    i => new SingleCartItemDto {
